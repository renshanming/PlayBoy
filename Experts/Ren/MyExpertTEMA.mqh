...
#include "MySignalTEMA.mqh"
#include "MyTrade.mqh"

class MyExpertTEMA
{
	MyTrade		trade;
	MySignalTEMA trendSignal;
public:
	MyExpertTEMA(void);
   	~MyExpertTEMA(void);
...
MyExpertTEMA::MyExpertTEMA(void)
{
...
bool MyExpertTEMA::Init(ulong magic_number, double lots, double take_profit, double stop_loss)
{
...
bool MyExpertTEMA::Processing(void)
{
	...
	if(trend != lastTrend)
	{
		if(lastTrend == TREND_DOWN && trend == TREND_UP)
		{
			trade.OrderOpen(MY_ORDER_BUY);
			trade.OrderClose(MY_ORDER_BUY);
		}
		if(lastTrend == TREND_UP && trend == TREND_DOWN)
		{
